...
{
    int cod_a_buscar,x=0,pos,sen=0,j;
    alumnos noinscriptos[1000];
    alumnos aux;
    FILE*fp1;
    FILE*fp2;
    FILE*fp3;
    ...
    while(cod_a_buscar!=0)
    {
        ...
        else
        {
            ...
            system("pause");
            noinscriptos[cant_noinscrp]=inscrip[x];
            fwrite(&noinscriptos[cant_noinscrp],sizeof(alumnos),1,fp3);
            cant_noinscrp++;
        }
        x++;
        do
    ...
    while(sen==0)
    {
        ...
        for(j=0;j<x-1;j++)
        {
            if(inscrip[j].cod_curso>inscrip[j+1].cod_curso)
            {
                aux=inscrip[j];
                inscrip[j]=inscrip[j+1];
                inscrip[j+1]=aux;
                sen=0;
                fwrite(&inscrip[j],sizeof(alumnos),1,fp2);
            }
         }
    }
    //---------------------------------------------------
...
{
    FILE*fp;
    int i,anterior,x=0,acumul=0,cantidades[20]={0},j=0;
    //-----------------------------------PREGUNTAR------------------------------
    fp=fopen("Inscripcion.dat","rb");
    if(fp==NULL)
    {
        printf("\n Hay un error con el archivo 1");
        exit(1);
    }
    fread(&inscrip[x],sizeof(alumnos),1,fp);
    while(!feof(fp))
    {
        acumul=0;
        anterior=inscrip[x].cod_curso;
        while(anterior==inscrip[x].cod_curso&&!feof(fp))
        {
            acumul+=inscrip[x].cant;
            x++;
            fread(&inscrip[x],sizeof(alumnos),1,fp);
        }
        cantidades[j]+=acumul;
...
void mostrarNoInscriptos(int cant)
{
    ...
    for(x=0;x<cant;x++)
    {
        fread(&noinscrip[x],sizeof(alumnos),1,fp4);
        printf("%s        %s          %d            %d\n",noinscrip[x].nombre,noinscrip[x].email,noinscrip[x].cod_curso,noinscrip[x].cant);
        printf("\n");
        system("Pause");
    }
    fclose(fp4);
